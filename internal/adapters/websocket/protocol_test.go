...
func TestDecodeMessageHelloMonitorMode(t *testing.T) {
	messageType, payload, err := decodeMessage([]byte(`{
        "type": "hello",
        "mode": "monitor",
        "client": "fixture-monitor",
        "version": "1.0"
    }`))
	if err != nil {
		t.Fatalf("decodeMessage returned error: %v", err)
	...
	if hello.Mode != contracts.ConnectionModeMonitor {
		t.Fatalf("expected monitor mode, got %q", hello.Mode)
	}

	if hello.Client != "fixture-monitor" {
		t.Fatalf("expected fixture monitor, got %q", hello.Client)
	}
}
